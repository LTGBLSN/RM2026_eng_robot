...
uint8_t rx_VTM_buff[VTM_ALL_BUFF_SIZE];
struct remote_data_t vtm_remoter;
...
/**
 * @brief 处理遥控器原生数据包 (21字节)
 * @param pData 接收缓冲区指针
 * @param size  HAL库返回的实际接收长度
 */
void Process_VTM_Data(uint8_t *pData, uint16_t size)
{
    // 1. 基本特征检查：必须是21字节，且以 A9 53 开头
    if (size == 21 && pData[0] == 0xA9 && pData[1] == 0x53)
    {
        // 2. 调用官方校验函数
        if (verify_crc16_check_sum(pData, 21))
        {
            // 3. 校验通过，直接映射到全局变量 vtm_remoter
