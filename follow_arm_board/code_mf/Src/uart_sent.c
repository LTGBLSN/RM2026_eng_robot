...
struct follow_arm_data arm_data;
// 2. 准备发送函数
void Send_Custom_Data(struct follow_arm_data *my_data)
{
    static uint8_t seq = 0;
    uint8_t send_buf[39]; // 总长度 39

    // --- 填充帧头 ---
    send_buf[0] = 0xA5;              // SOF
    send_buf[1] = 30 & 0xFF;         // Data长度低位
    send_buf[2] = (30 >> 8) & 0xFF;  // Data长度高位
    send_buf[3] = seq++;             // 包序号
    Append_CRC8_Check_Sum(send_buf, 5); // 计算并填入 CRC8 (需根据附录一实现)

    // --- 填充命令码 ---
    send_buf[5] = 0x02; // cmd_id 0x0302 低位
    send_buf[6] = 0x03; // cmd_id 0x0302 高位

    // --- 填充数据段 ---
    // 先清空 30 字节 Data 区域
    memset(&send_buf[7], 0, 30);
    // 将你的 22 字节结构体拷贝进去
    memcpy(&send_buf[7], my_data, sizeof(struct follow_arm_data));

    // --- 填充帧尾 ---
    Append_CRC16_Check_Sum(send_buf, 39); // 计算并填入整包 CRC16

    // --- 物理发送 ---
    HAL_UART_Transmit(&huart1, send_buf, 39, 10);
}
